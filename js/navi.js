...
var currentposRange = [];
currentposRange[0] = 0;
var lunbosceneHeight = document.getElementsByClassName(
  "box-lunboscene-frame"
)[0].offsetHeight;
...
var dinnerHeight =
  document.getElementsByClassName("box-dinnerscene")[0].offsetHeight;
...
var questionHeight =
  document.getElementsByClassName("box-questionnaire")[0].offsetHeight;
currentposRange[6] = currentposRange[5] + questionHeight;
var drawingHeight =
  document.getElementsByClassName("box-drawing")[0].offsetHeight;
...
var RemCatchHeight =
  document.getElementsByClassName("box-RemCatch")[0].offsetHeight;
...
for (let i = 0; i < naviItems.length; i++) {
  naviItems[i].onclick = function () {
    new TWEEN.Tween({ pos: all.scrollTop })
      .to(
        { pos: currentposRange[i] },
        Math.abs(all.scrollTop - currentposRange[i]) / 10
      )
...
function findCurrentOption() {
  if (all.scrollTop <= currentposRange[1]) {
    handleCurrentOptionchange(1);
    currentOption = 1;
  } else if (all.scrollTop <= currentposRange[2]) {
    handleCurrentOptionchange(2);
    currentOption = 2;
  } else if (all.scrollTop <= currentposRange[3]) {
    handleCurrentOptionchange(3);
    currentOption = 3;
  } else if (all.scrollTop <= currentposRange[4]) {
    handleCurrentOptionchange(4);
    currentOption = 4;
  } else if (all.scrollTop <= currentposRange[5]) {
    handleCurrentOptionchange(5);
    currentOption = 5;
  } else if (all.scrollTop <= currentposRange[6]) {
    handleCurrentOptionchange(6);
    currentOption = 6;
  } else if (all.scrollTop <= currentposRange[7]) {
    handleCurrentOptionchange(7);
    currentOption = 7;
  } else if (all.scrollTop <= currentposRange[8]) {
    handleCurrentOptionchange(8);
    currentOption = 8;
  } else if (all.scrollTop <= currentposRange[9]) {
    handleCurrentOptionchange(9);
    currentOption = 9;
  } else {
    currentOption = -1;
  }
...
function naviLoop() {
  ...
  if (lastscroll > all.scrollTop) {
    document.getElementsByClassName("navi")[0].style.transform =
      "translateY(0vh)";
...
window.onload = function () {
  ...
  for (let i = 0; i < searchURL.split("&").length; i++) {
    if (searchURL.split("&")[i].split("=")[0] === "to") {
      let target = document.getElementsByClassName(
        "box-" + searchURL.split("&")[i].split("=")[1]
      )[0];
      if (target) {
        new TWEEN.Tween({ pos: all.scrollTop })
          .to(
            { pos: target.offsetTop },
            Math.abs(all.scrollTop - target.offsetTop) / 10
          )
          .onUpdate(function (obj) {
            all.scrollTop = obj.pos + 1;
          })
