...
function handleChangeThead(mode, thinnerHTMLs) {
  let elemId;
  if (mode == "enroll") {
    elemId = "inquiryTableTheadTr";
  } else if (mode == "drop") {
    elemId = "dropTableTheadTr";
  } else if (mode == "schedule") {
    elemId = "scheduleTableTheadTr";
  }
...
function menucontentHandler(user, deltaY) {
  if (!menuMasked) {
    document.getElementsByClassName("menu-mask")[
      user
    ].style.transform = `translateY(-${deltaY}vh)`;
    menuMasked = true;
  } else {
    document.getElementsByClassName("menu-mask")[
      user
...
document.getElementById("menu-student-title").onclick = function () {
  menucontentHandler(0, 10.5);
};
document.getElementById("menu-teacher-title").onclick = function () {
  menucontentHandler(1, 3.5);
};
document.getElementById("menu-admin-title").onclick = function () {
  menucontentHandler(2, 10.5);
};
...
function handleScheduleQuery(quiryType) {
  ...
  if (tested) {
    ...
    if (quiryType == 1) {
      ...
      xhr.open("POST", `${flaskurl}/get_student_schedule/`, true);
    }
  }
  xhr.setRequestHeader("Content-Type", "application/json");
  xhr.setRequestHeader("Authorization", Authorization);
...
function appendIdSelect() {
  ...
  newbutton.onclick = function () {
    ...
    if (tested) {
      ...
      xhr.open("POST", `${flaskurl}/identify_operated_user/`, true);
    }
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.setRequestHeader("Authorization", Authorization);
    if (tested) {
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState == 4) {
        ...
        if (res.status == "success") {
          if (res.user_info.role == "1") {
            adminChoose = 1;
            showScheduleInquiry(3, res.course_info, [
              "序号",
              "课程号",
              "课程名",
              "学分",
          ...
          if (res.user_info.role == "2") {
            adminChoose = 2;
            showScheduleInquiry(4, res.course_info, [
              "序号",
              "课程号",
              "课程名",
              "学分",
  ...
  for (let i = 0; i < courseInfo.length; i++) {
    ...
    for (let j = 0; j < 5; j++) {
      if (courseInfo[i].sksj[0] == week[j]) {
        ...
        let leg = end - start + 1;
        for (let k = 0; k < leg; k++) {
          document.getElementsByClassName("schedule-item")[
            j + (start - 1 + k) * 5
          ].innerHTML = xuhao[i];
        }
      }
...
function maskHandler() {
  ...
  newdiv.style.position = "absolute";
  newdiv.style.left = "1vw";
  newdiv.style.top = "14vh";
  newdiv.style.width = "80vw";
  newdiv.style.height = "3vw";
  newdiv.style.backgroundColor = "#fbf9f9";
  document.getElementsByClassName("box-inquiry-input")[0].appendChild(newdiv);
}
